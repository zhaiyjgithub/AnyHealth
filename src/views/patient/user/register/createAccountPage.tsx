...
export default function CreateAccountPage() {
        ...
        <div className={"w-full"}>
            ...
            <div className={"w-full flex flex-row items-center bg-white border"}>
                <div className={"px-3 h-12 flex flex-1 flex-row"}>
                    <FormRadio titleClassName={"text-base"} key={3} title={"Male"} checked={profile.gender === "F"}
                        onChange={() => {
                            setProfile({
                                ...profile,
                                gender: "F",
                ...
                <div className={"px-3 h-12 flex flex-1 flex-row border-l"}>
                    <FormRadio titleClassName={"text-base"} key={4} title={"Female"} checked={profile.gender === "M"}
                        onChange={() => {
                            setProfile({
                                ...profile,
                                gender: "M",
    ...
    const $errMsgForEmail = isEmailInvalid ?
        <p className={"text-xs text-red-500 font-medium"}>Oops! Try a valid email.</p> : null
    const $emailForm = (
        <div className={"w-full"}>
    ...
    const $errMsgForConfirmEmail = !isEmailInvalid && check.confirmEmail && profile.email !== profile.confirmEmail ?
        <p className={"text-xs text-red-500 font-medium"}>The confirm email must match your email address.</p> : null
    ...
    const $validatorView = check.password ? (
        <div className={"space-y-2 mt-2"}>
            <div className={"flex flex-row items-center space-x-2"}>
                ...
                <p className={`text-xs ${checkPasswordLength ? "text-green-500" : "text-red-500"} font-semibold`}>Use at
                    least 8 characters</p>
            </div>

            <div className={"flex flex-row items-center space-x-2"}>
                <i className={`fas fa-check-double text-xs ${checkPasswordLetters ? "text-green-500" : "text-red-500"}`}/>
                <p className={`text-xs ${checkPasswordLetters ? "text-green-500" : "text-red-500"} font-semibold`}>Use
                    letters</p>
            </div>

            <div className={"flex flex-row items-center space-x-2"}>
                <i className={`fas fa-check-double text-xs ${checkPasswordNumbers ? "text-green-500" : "text-red-500"}`}/>
                <p className={`text-xs ${checkPasswordNumbers ? "text-green-500" : "text-red-500"} font-semibold`}>Use
                    numbers</p>
            </div>

            <div className={"flex flex-row items-center space-x-2"}>
                <i className={`fas fa-check-double text-xs ${checkPasswordSpecialChars ? "text-green-500" : "text-red-500"}`}/>
                <p className={`text-xs ${checkPasswordSpecialChars ? "text-green-500" : "text-red-500"} font-semibold`}>Use
    ...
    const $agreementView = (
        <div className={"flex flex-row space-x-1 items-center"}>
            ...
            <p className={"text-sm font-medium text-primary-focus"}>
                ...
                <a className={"mx-1 cursor-pointer leading-none border-b border-primary-focus hover:border-blue-600 hover:text-blue-600"}>Terms
                    of Use</a>
                {"and"}
                <a className={"mx-1 cursor-pointer leading-none border-b border-primary-focus hover:border-blue-600 hover:text-blue-600"}>Privacy
                    Policy</a>
            </p>
        </div>
    )
    ...
    const onCreateUser = () => {
        if (!profile.checkedAgreement) {
            alert("Please selected the agreement")
            return
        }
        const date = `${profile.birthdayYear}-${profile.birthdayMonth}-${profile.birthdayDay}`
