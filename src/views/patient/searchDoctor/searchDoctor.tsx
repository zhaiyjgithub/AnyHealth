...
export default function SearchDoctor() {
    ...
    const $appointmentTypeTabList = (
        <div className={"flex fle-row items-center space-x-6 mt-6 px-6"}>
            ...
            className={`inline-block font-semibold py-1 text-primary-focus hover:text-gray-400 text-base font-medium border-b-2 ${apptType === AppointmentType.anyType ? "border-primary-focus" : "border-transparent"}`}>
                All appointments
            </button>
            <button onClick={() => {
            ...
            className={`inline-block font-semibold py-2 text-primary-focus hover:text-gray-400 text-base font-medium border-b-2 ${apptType === AppointmentType.inClinic ? "border-primary-focus" : "border-transparent"}`}>
                In-person
            </button>
            <div className={"flex flex-row items-center space-x-2"}>
                ...
                className={`inline-flex font-semibold flex-row items-center space-x-2 py-2 text-primary-focus hover:text-gray-400 text-base font-medium border-b-2 ${apptType === AppointmentType.virtual ? "border-primary-focus" : "border-transparent"}`}>
                    <p>Video visit</p>
                </button>
    ...
    const $allTimeSlotsModal = viewAllIdx !== -1 && data.length ? (
        <AllAvailableTimeSlots timeSlotsPerDay={dataForAllAvailable} show={viewAllIdx !== -1}
            doctorInfo={data[viewAllIdx]} onClose={onCloseViewAllAvailableTimeSlotsModal}
            onRequestTimeSlots={(date) => {
    ...
    const pins = useMemo(() => {
        return data.map((doctorInfo) => {
            ...
            return {
                ...
                specialty: specialty,
                address: address,
                pos: [doctorInfo.location.lat, doctorInfo.location.lon],
            } as Pin
        })
    }, [data])
