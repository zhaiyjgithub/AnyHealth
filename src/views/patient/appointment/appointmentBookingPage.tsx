...
export default function AppointmentBookingPage() {
    ...
    const {
        npi, date, offset, appointmentType,
    ...
    const $addPhoneNumberButton = (
        <Button onClick={onAddPhoneNumber}
            status={ButtonStatus.link}>{phoneNumber.length ? phoneNumber : "Add phone number"}</Button>
    )
    ...
    const $dropdownForInsurance = (
        <Dropdown title={"Insurance"} placeholder={"Choose an insurance"} selected={insurance} data={dataForInsurance}
            onChange={(_id) => {
                setInsurance(_id)
            }}/>
    )
    ...
    const $dropdownForIllness = (
        <Dropdown title={"What's the reason for your visit?"} placeholder={"Illness"} selected={illness}
            data={dataForIllness} onChange={(_id) => {
                setIllness(_id)
            }}/>
    )
    ...
    const $agreementView = (
        <div className={"flex flex-row space-x-2 cursor-pointer"}>
            ...
            className={"form-checkbox mt-1 w-4 h-4 rounded-none flex-none transition duration-150"}/>
