...
export default function TimeSlotsView(props: IProps) {
    ...
    const $previous = (
        ...
        className={`${isToday ? "disabled" : "hover:bg-base-300"} h-8 w-8 flex items-center justify-center rounded-full`}>
    ...
    const $moreItem = (
        <button type={"button"}
            className={"w-full py-2 border bg-white hover:bg-primary-focus text-primary-focus hover:text-focus leading-snug text-sm font-medium"}
            onClick={() => {
    ...
    const $timeSlot = (timeSlot: TimeSlot, idx: number) => {
        ...
        return (
            <button type={"button"}
                className={`relative w-full py-2 border ${isSelected ? "bg-primary-focus text-focus" : "bg-white hover:bg-primary-focus text-primary-focus hover:text-focus"} font-medium leading-snug text-sm`}
                key={idx} onClick={() => {
