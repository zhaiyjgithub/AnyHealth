...
export default function AppointmentInfoPanel(props: IProps) {
    ...
    const $dropdownForInsurance = (
        <Dropdown errMsg={errMsgForInsurance} title={"What's your insurance plan?"} placeholder={"Choose an insurance"}
            selected={booking.insuranceId} data={dataForInsurance} onChange={(id) => {
                setBooking({
                    ...booking,
                    insuranceId: id,
                })
    ...
    const $dropdownForIllness = (
        <Dropdown errMsg={errMsgForIllness} title={"What's the reason of your visit?"} placeholder={"Illness"}
            selected={booking.illnessId} data={dataForIllness} onChange={(id) => {
                setBooking({
                    ...booking,
                    illnessId: id,
                })
    ...
    const $appointmentTypeOptionView = (
        <div className={"w-full"}>
            ...
            <div className={"flex flex-row items-center justify-between border mt-1 bg-white"}>
                <div className={"px-3 py-2.5 flex flex-1 flex-row"}>
                    <FormRadio key={1} title={"In-person"}
                        checked={booking.appointmentType === AppointmentType.inClinic} onChange={() => {
                            setBooking({
                                ...booking,
                                appointmentType: AppointmentType.inClinic,
                            })
                ...
                <div className={"px-3 py-2.5 flex flex-1 flex-row border-l"}>
                    <FormRadio key={2} title={"Video visit"}
                        checked={booking.appointmentType === AppointmentType.virtual} onChange={() => {
                            setBooking({
                                ...booking,
