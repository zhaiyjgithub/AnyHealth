...
export default function LocationInfo(props: IProps) {
    ...
    const $addressView = (
        <div className={"w-1/5 flex flex-col flex-none space-y-4 mt-4"}>
            ...
            <button type={"button"}
                className={"max-w-max border-b leading-none border-blue-500 border-dotted hover:border-solid text-lg text-blue-500 p-0"}>
                Get Directions
            </button>
        </div>
    )
    const $mapInfoView = (
        <div className={"flex flex-row space-x-4"}>
            <div className={"flex flex-1 h-80 z-10"}>
                <LeafletMap pins={[{name: doctorName, specialty: specialty, address: address, pos: center}]} zoom={20}
                    center={center}/>
            </div>
            {$addressView}
        </div>
